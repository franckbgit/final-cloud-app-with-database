...
def submit(request, course_id):
    ...
    #careful set deletes all existing associated models
    #we could loop and use add or use add(*submitted_answers)
    submission.choices.set(submitted_answers)
    submission.save()
    #need to finish with http response
    #return HttpResponseRedirect(reverse(viewname='onlinecourse:course_details', args=(course.id,)))
...
def show_exam_result(request, course_id, submission_id):
    ...
    for choice in choices:
        if choice.is_correct:
            # I guess we should adjust the grade for any good answer
            # It is a design choice that simplifies implementation
            # I decide to do this
